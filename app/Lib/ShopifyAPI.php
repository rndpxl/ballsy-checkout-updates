...
//		$defaultHeaders[] = 'Content-length: 0';
		...
		$options = array(
			...
			CURLOPT_ENCODING        => '',
			CURLOPT_USERAGENT       => 'RPS Shopify API Wrapper',
			CURLOPT_FAILONERROR     => $request['FAILONERROR'],
			CURLOPT_VERBOSE         => $request['ALLDATA'],
			CURLOPT_HEADER          => 1
		);

		if (isset($request['CURL']))
		{
			foreach($request['CURL'] as $k => $v)
			{
				$options[$k] = $v;
			}
		}
		...
		if (!empty($request['DATA']))
		{
			if (is_array($request['DATA']))
			{
				...
				{
					$options[CURLOPT_URL] .= '?' . http_build_query($request['DATA']);
				}
			}
			else
			{
